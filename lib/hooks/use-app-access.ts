...
export function useGrantAppAccess(appId: string) {
  ...
  return useMutation({
    mutationFn: (data: appsApi.GrantAppAccessDto) =>
      appsApi.grantAppAccess(appId, data),
    onSuccess: (response) => {
      ...
      permissions.forEach((permission) => {
        queryClient.invalidateQueries({
          queryKey: appsKeys.userApps(permission.userId),
        });
      });
...
export function useUpdateAppAccess(appId: string) {
  ...
  return useMutation({
    mutationFn: ({
      ...
      userId: string;
      data: appsApi.UpdateAppAccessDto;
...
/**
 * Hook to create a new Mendix app with full integration
 * - Creates Mendix project via Build API
 * - Deploys app via Deploy API to get AppId (subdomain)
 * - Creates GitHub repository (if org has GitHub integration)
 * - Performs initial sync via Model SDK
 */
export function useCreateMendixApp() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (data: appsApi.CreateMendixAppDto) =>
      appsApi.createMendixApp(data),
    onSuccess: async (response) => {
      const app = response.data;
      // Invalidate all app-related queries
      await queryClient.invalidateQueries({ queryKey: ["apps"] });
      ...
      if (app.status === "synced") {
        toast.success("Mendix app created and synced!", {
          description: `${app.name} is ready. GitHub repository: ${
            app.githubRepoUrl ? "created" : "not connected"
          }`,
        });
      } else if (app.status === "deployed") {
        toast.success("Mendix app created!", {
          description:
            app.syncMessage || `${app.name} is deployed and ready to use`,
        });
      } else {
        toast.warning("Mendix app created (needs deployment)", {
          description: `${app.name} was created but needs manual deployment in Mendix Portal`,
        });
      }
    },
    onError: (error: any) => {
      ...
      toast.error("Failed to create Mendix app", {
        description: message,
      });
    },
  });
