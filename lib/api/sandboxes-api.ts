...
export const sandboxesApi = {
  ...
  async list(filters?: SandboxFilters): Promise<ListSandboxesResponse> {
    const response = await apiClient.get<ListSandboxesResponse>(SANDBOXES_BASE, {
      params: filters,
    });
    return response.data;
  },
  ...
  async listMy(): Promise<ListSandboxesResponse> {
    const response = await apiClient.get<ListSandboxesResponse>(`${SANDBOXES_BASE}/my`);
    return response.data;
  },
  ...
  async update(id: string, data: UpdateSandboxRequest): Promise<Sandbox> {
    const response = await apiClient.patch<Sandbox>(`${SANDBOXES_BASE}/${id}`, data);
    return response.data;
  },
  ...
  async getStats(id: string): Promise<SandboxStats> {
    const response = await apiClient.get<SandboxStats>(`${SANDBOXES_BASE}/${id}/stats`);
    return response.data;
  },
   ...
   * This is a convenience method that calls the backend to check quota
   */
  async checkQuota(): Promise<QuotaCheckResponse> {
    // Note: This endpoint doesn't exist in the backend yet, but it's useful
    // For now, we'll handle quota checking on the frontend by counting sandboxes
    try {
      const { data, total } = await this.listMy();
      const maxAllowed = 5; // Default max, should come from backend eventually
      return {
